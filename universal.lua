...
local autoInteractRadius = 15
local toolMagnetEnabled = false
...
    task.wait(0.5)
    if ghostModeEnabled then ghostModeEnabled = false end 
...
                if not fovCircleEnabled or dist <= fovRadius then
...
local WorldTab = Window:CreateTab("World & Advantages", 4483362458)
...
WorldTab:CreateSection("Exploration & Automation")

local spectateDropdown = WorldTab:CreateDropdown({
...
        if p ~= LocalPlayer then table.insert(pNames, p.Name) end
    end
    spectateDropdown:Refresh(pNames)
end
...
WorldTab:CreateToggle({ Name = "Anti-Fling (Disable Player Collision)", CurrentValue = false, Flag = "AntiFlingToggle", Callback = function(Value) antiFlingEnabled = Value end})
WorldTab:CreateToggle({ Name = "Tool & Drop Magnet", CurrentValue = false, Flag = "ToolMagnetToggle", Callback = function(Value) toolMagnetEnabled = Value end})
WorldTab:CreateToggle({ Name = "Auto-Trigger Nearby Prompts", CurrentValue = false, Flag = "AutoInteractToggle", Callback = function(Value) autoInteractEnabled = Value end})
WorldTab:CreateSlider({ Name = "Auto-Trigger Radius", Range = {5, 50}, Increment = 1, Suffix = "Studs", CurrentValue = 15, Flag = "InteractRadiusSlider", Callback = function(Value) autoInteractRadius = Value end})
WorldTab:CreateToggle({ Name = "Auto-Jump (Edge Detection)", CurrentValue = false, Flag = "AutoJumpToggle", Callback = function(Value) autoJumpEnabled = Value end})

WorldTab:CreateSection("Map Modification")
WorldTab:CreateButton({ Name = "Destroy All Killbricks", Callback = function() 
    local count = 0
    for _, v in pairs(workspace:GetDescendants()) do
        if v:IsA("TouchTransmitter") and v.Parent and not v.Parent:IsDescendantOf(LocalPlayer.Character) then
            if v.Parent.Name:lower():match("kill") or v.Parent.Name:lower():match("lava") or v.Parent.BrickColor.Name == "Bright red" then
                v:Destroy()
                count = count + 1
            end
        end
    end
    Rayfield:Notify({Title="Map Cleansed", Content="Destroyed " .. count .. " potential killbricks.", Duration=3})
end})

WorldTab:CreateButton({ Name = "Save Current Location", Callback = function() 
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        savedWaypointCFrame = LocalPlayer.Character.HumanoidRootPart.CFrame
        Rayfield:Notify({Title="Waypoint Saved", Content="Your coordinates have been locked.", Duration=2})
    end
end})

WorldTab:CreateButton({ Name = "Teleport to Saved Location", Callback = function() 
    if savedWaypointCFrame and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        LocalPlayer.Character.HumanoidRootPart.CFrame = savedWaypointCFrame
        Rayfield:Notify({Title="Teleported", Content="Returned to saved waypoint.", Duration=2})
    else
        Rayfield:Notify({Title="Error", Content="No waypoint saved yet.", Duration=2})
    end
end})
...
CombatTab:CreateToggle({ Name = "Show Aimbot FOV Circle", CurrentValue = false, Flag = "FOVCircleToggle", Callback = function(Value) 
    fovCircleEnabled = Value
    FOVCircle.Visible = Value
end})
CombatTab:CreateSlider({ Name = "Aimbot FOV Radius", Range = {10, 800}, Increment = 10, Suffix = "px", CurrentValue = 150, Flag = "FOVRadiusSlider", Callback = function(Value) 
    fovRadius = Value
...
CombatTab:CreateToggle({ Name = "Kill Aura (Uses Equipped Tool)", CurrentValue = false, Flag = "KillAuraToggle", Callback = function(Value) killAuraEnabled = Value end})
CombatTab:CreateSlider({ Name = "Kill Aura Range", Range = {5, 50}, Increment = 1, Suffix = "Studs", CurrentValue = 15, Flag = "KillAuraRangeSlider", Callback = function(Value) killAuraRange = Value end})
CombatTab:CreateToggle({ Name = "Auto Clicker [Hold Left Click]", CurrentValue = false, Flag = "AutoClickToggle", Callback = function(Value) autoClickerEnabled = Value end})
...
UtilsTab:CreateToggle({ Name = "Instant Proximity Prompts", CurrentValue = false, Flag = "InstantPromptsToggle", Callback = function(Value) instantPromptsEnabled = Value end})
...
    if toolMagnetEnabled and myHRP then
...
    if autoJumpEnabled and myHum and myHum.FloorMaterial ~= Enum.Material.Air then
...
    if autoClickerEnabled and isHoldingLMB then
        VirtualUser:ClickButton1(Vector2.new())
    end
    
    if killAuraEnabled and myChar then
        local tool = myChar:FindFirstChildOfClass("Tool")
        if tool and tool:FindFirstChild("Handle") then
            for _, p in pairs(Players:GetPlayers()) do
                if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") and p.Character:FindFirstChild("Humanoid") and p.Character.Humanoid.Health > 0 then
                    if (p.Character.HumanoidRootPart.Position - myHRP.Position).Magnitude <= killAuraRange then
